...
# Fonction pour obtenir le répertoire d'origine du script
def get_origin_path():
	return os.path.dirname(os.path.realpath(__file__))

# Fonction pour traduire un texte avec DeepL
def translate_with_deepl(json_content, source_lang, target_lang, auth_key):
	translator = deepl.Translator(auth_key)

	# Translate each value in the JSON content
	translated_json = {}
	for key, value in json_content.items():
		translated_value = translator.translate_text(value, source_lang=source_lang, target_lang=target_lang)
		translated_json[key] = translated_value.text

	return translated_json

# Fonction pour traduire un fichier JSON
def translate_json_file(input_file, source_lang, target_lang, auth_key, count_file, shell_script):
	with open(input_file, "r") as f:
		json_content = json.load(f)

	translated_json = translate_with_deepl(json_content, source_lang, target_lang, auth_key)

	# Calculer le nombre de caractères dans le texte source
	current_character_count = get_character_count(count_file)

	# Convert the translated JSON back to a string for character count calculation
	translated_text = json.dumps(translated_json, ensure_ascii=False)
	character_count = len(translated_text.split())
	...
	if current_character_count >= character_count:
		...
		# Obtenez le répertoire d'origine du script
		script_origin_path = get_origin_path()

		# Créer le chemin du répertoire de résultat dans le répertoire d'origine
		result_dir = os.path.join(script_origin_path, "result")

		os.makedirs(result_dir, exist_ok=True)  # Créer le répertoire s'il n'existe pas

		# Créer le nom du fichier traduit dans le répertoire de résultat
		output_file = os.path.join(result_dir, f"{file_name}_{target_lang}.json")
...
def main():
	...
	if os.path.isdir(input_path):
		# Si c'est un répertoire, parcourir les fichiers et traduire chaque fichier JSON
		for root, _, files in os.walk(input_path):
			for file in files:
				if file.endswith(".json"):  # Update the file extension check to ".json"
					translate_json_file(os.path.join(root, file), source_lang, target_lang, DEEPL_AUTH_KEY, count_file, shell_script)
	else:
